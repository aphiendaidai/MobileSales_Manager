...
import Mobile_manager.CTL.GiamGiaCTL;
import Mobile_manager.Model.GiamGia;

public class DlgTimMaGiam extends JDialog {

    public static GiamGia maGiamTimDuoc = null;
    private GiamGiaCTL giamGiaBUS = new GiamGiaCTL();
...
private void xuLyChonMaGiam() {
    ...
    if (row > -1) {
        try {
            ...
            // Sử dụng phương thức parseNumber để xử lý chuỗi số
            int ma = parseNumber(tblMaGiam.getValueAt(row, 0).toString());
            String ten = tblMaGiam.getValueAt(row, 1).toString();
            int phanTram = parseNumber(tblMaGiam.getValueAt(row, 2).toString());
            String dieuKienst = tblMaGiam.getValueAt(row, 3).toString();
            dieuKienst = dieuKienst.replace(">", "").replace(",", "");
            int dieuKien = parseNumber(dieuKienst);
            ...
            SimpleDateFormat sdf = new SimpleDateFormat("MM/dd/yyyy");
            Date ngayBD = sdf.parse(tblMaGiam.getValueAt(row, 4).toString());
            Date ngayKT = sdf.parse(tblMaGiam.getValueAt(row, 5).toString());
            ...
            ex.printStackTrace();
            new MyDialog("Có lỗi xảy ra khi xử lý mã giảm giá!", MyDialog.ERROR_DIALOG);
        }
    }
    xuLyThoat();
}

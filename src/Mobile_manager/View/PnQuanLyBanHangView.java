...
import Mobile_manager.CTL.CTHoaDonCTL;
import Mobile_manager.CTL.DangNhapCTL;
import Mobile_manager.CTL.HoaDonCTL;
import Mobile_manager.CTL.LoaiCTL;
import Mobile_manager.CTL.NhanVienCTL;
import Mobile_manager.CTL.SanPhamCTL;
import Mobile_manager.Model.CTHoaDon;
import Mobile_manager.Model.HoaDon;
import Mobile_manager.Model.LoaiSP;
import Mobile_manager.Model.NhanVien;
import Mobile_manager.Model.SanPham;

import java.text.DecimalFormat;

public class PnQuanLyBanHangView extends JPanel {

    private SanPhamCTL spBUS = new SanPhamCTL();
    private NhanVienCTL nvBUS = new NhanVienCTL();
    private LoaiCTL loaiBUS = new LoaiCTL();
    private HoaDonCTL hoaDonBUS = new HoaDonCTL();
    ...
    lblNhanVien.setPreferredSize(new Dimension(lblNhanVien.getPreferredSize().width + 50, lblNhanVien.getPreferredSize().height + 20));
    ...
    private void loadDataComboboxNhanVienBan() {
        ...
        for (int i = 0; i < cmbNhanVienBan.getItemCount(); i++) {
            ...
            if (cmbValue[0].equals(DangNhapCTL.taiKhoanLogin.getMaNhanVien() + "")) {
    ...
    private int parseIntWithDot(String input) {
        input = input.replace(".", ""); // Loại bỏ dấu chấm
        return Integer.parseInt(input); // Chuyển đổi thành số nguyên
    }
    ...
    private void xuLyClickTblBanHang() {
        ...
        if (row > -1) {
            try {
                ...
                String donGiaStr = tblBanHang.getValueAt(row, 2) + "";
                String soLuongStr = tblBanHang.getValueAt(row, 3) + "";
                String anh = tblBanHang.getValueAt(row, 5) + "";

                // Sử dụng phương thức parseIntWithDot để xử lý chuỗi số
                int donGia = parseIntWithDot(donGiaStr);
                int soLuong = parseIntWithDot(soLuongStr);
                ...
                txtDonGiaBanHang.setText(String.valueOf(donGia));
                loadAnh(anh);
            } catch (NumberFormatException e) {
                MyDialog dlg = new MyDialog("Lỗi định dạng số: " + e.getMessage(), MyDialog.ERROR_DIALOG);
            }
        }
    }
...
//    }
    ...
    private void xuLyThemVaoGioHang() {
        ...
        String donGiaStr = txtDonGiaBanHang.getText();
        int soLuong = Integer.parseInt(spnSoLuongBanHang.getValue().toString());
        
        // Sử dụng phương thức parseIntWithDot để xử lý chuỗi số có dấu chấm
        int soLuongConLai = parseIntWithDot(tblBanHang.getValueAt(row, 3).toString());
        ...
        int key = parseIntWithDot(ma);
        for (int i = 0; i < tblGioHang.getRowCount(); i++) {
            int maTbl = parseIntWithDot(tblGioHang.getValueAt(i, 0).toString());
            if (maTbl == key) {
                int soLuongAdd = parseIntWithDot(tblGioHang.getValueAt(i, 2).toString());
                soLuongAdd += soLuong;

                // Loại bỏ dấu phân cách hàng nghìn trong donGiaStr
                donGiaStr = donGiaStr.replace(".", "");
                int donGia = Integer.parseInt(donGiaStr);

                tblGioHang.setValueAt(soLuongAdd, i, 2);
                tblGioHang.setValueAt(dcf.format(soLuongAdd * donGia), i, 4);
        ...
        donGiaStr = donGiaStr.replace(".", "");
        int donGia = Integer.parseInt(donGiaStr);
        vec.add(dcf.format(soLuong * donGia));
...
//
    ...
    private void xuLyXuatHoaDonBanHang() {
        ...
        for (int i = 0; i < row; i++) {
            ...
            // Lấy giá trị từ cột tổng tiền, loại bỏ dấu phân cách và chuyển đổi thành số nguyên
            String tongTienStr = tblGioHang.getValueAt(i, 4).toString().replace(".", "").replace(",", "");
            try {
                tongTien += Integer.parseInt(tongTienStr);
            } catch (NumberFormatException e) {
                e.printStackTrace();
                new MyDialog("Lỗi chuyển đổi giá trị tổng tiền: " + tongTienStr, MyDialog.ERROR_DIALOG);
                return;
            }
        ...
        XuatHoaDonView hoaDonUI = new XuatHoaDonView(dsGioHang, tongTien, cmbNhanVienBan.getSelectedItem());
    ...
    private void addDataListHoaDon(ArrayList<HoaDon> dshd) {
        ...
        if (dshd != null) {
            for (HoaDon hd : dshd) {
                listModel.addElement(hd.getMaHD() + " | " + hd.getNgayLap() + " === " + dcf.format(hd.getTongTien()) + "VNĐ");
            }
    ...
    private CTHoaDonCTL ctHDBUS = new CTHoaDonCTL();
    ...
    private void xuLyClickTblCTHoaDon() {
        ...
        if (row > -1) {
            ...
            // Lấy đối tượng SanPham, kiểm tra null trước khi gọi getTenSP()
            SanPham sanPham = spBUS.getSanPham("" + tblCTHoaDon.getValueAt(row, 1));
            String maSP;
            if (sanPham != null) {
                maSP = sanPham.getTenSP();
            } else {
                maSP = "N/A";  // Hoặc giá trị mặc định nào đó khi sản phẩm không tồn tại
            }

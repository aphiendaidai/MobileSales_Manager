...
public class XuatHoaDonView extends JDialog {
    ...
    public XuatHoaDonView(ArrayList<Vector> dsGioHang, int tongTien, Object nhanVien) {
        this();
...
//    }
    private void checkKhachMa() {
        boolean tenKhachRong = txtTenKhach.getText().isEmpty();
        boolean maGiamRong = txtMaGiam.getText().isEmpty();
        
        btnThanhToan.setEnabled(!tenKhachRong && !maGiamRong);
    }
...
//    }
    ...
    private void btnThanhToanActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        for (Vector vec : dsGioHang) {
            ...
            String soLuong = vec.get(2).toString();
            
            // Loại bỏ dấu phân cách trong donGia và thanhTien
            String donGia = vec.get(3).toString().replace(".", "").replace(",", "");
            String thanhTien = vec.get(4).toString().replace(".", "").replace(",", "");
           ...
           // ctHoaDonBUS.addCTHoaDon(maSP, soLuong, donGia, thanhTien);
            ctHoaDonBUS.addCTHoaDon(maSP, soLuong, Integer.toString(donGiaInt), Integer.toString(thanhTienInt));
